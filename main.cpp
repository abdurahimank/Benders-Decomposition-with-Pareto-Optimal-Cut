...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		obj_aux_sub.end();
		cplex_aux_sub.setOut(env.getNullStream()); // This is to supress the output of Branch & Bound Tree on screen
		cplex_aux_sub.setWarning(env.getNullStream()); //This is to supress warning messages on screen
		...
		//while( Iter < MaxCut )
		while (Upper_bound - Lower_bound > eps)  // Since stopping criteria is duality gap < eps.
		{
			Iter++;
			cout << "=========================================" << endl;
			...
			{// Dual subproblem is bounded; Add Optimality Cut to the Master Problem
				...
				cout << "Auxiliary Subproblem vaues" << D_aux_val << endl;
